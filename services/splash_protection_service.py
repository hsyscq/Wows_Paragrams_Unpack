...
背景（本仓库已逆向确认，见 memory he-splash-module-protection.md）：
  游戏内 `PySplashMesh::getSplashEffectiveArmor`（Korabli64.exe FUN_140581000）：
...
说明：`getSplashEffectiveArmor` 的精确 thk 由游戏 C++ 在加载 splash mesh 时赋予，
...
# ── 射线-三角形求交（游戏 getSplashEffectiveArmor 逐面防护板依据）─────
...
def _module_effective_armor(box_names, splash_boxes, mesh) -> tuple[Optional[float], Optional[float]]:
    ...
    依据（本仓库逆向确认，见 memory he-splash-module-protection.md）：
      游戏 `getSplashEffectiveArmor` 每盒有 6 个面厚度 thk（x±/y±/z±），
